...
def add_routes(app):
    ...
    def add_boat():
        boat = Boat(name="Kalle", type=BoatType.C8_DC, owner="Kalle Anka")
        schema = BoatSchema()
    ...
    def get_boat_by_type(boat_type: str):
        boats = [
            Boat(name=f"{boat_type}_{i}", type=BoatType.C8_DC, owner="Kalle Anka")
            for i in range(10)
        ]
    ...
    def get_boats():
        boats = [
            Boat(name=f"boat_{i}", type=BoatType.C8_DC, owner="Kalle Anka")
            for i in range(10)
        ]
        schema = BoatSchema(many=True)
        return jsonify(schema.dump(boats)), 200
